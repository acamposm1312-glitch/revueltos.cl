...
import { PILDORAS_EDUCATIVAS } from '../src/core/contenido.js';
...
const SIN_TILDE = [
  ...
  'ano ', 'anos', 'manana', 'confianza',
];

/**
 * Los marcadores {{variable}} son nombres de campo, no texto que lea el
 * cliente, asi que se quitan antes de revisar la ortografia.
 */
const buscarSinTilde = (texto) => {
  const plano = String(texto).replace(/\{\{[^}]*\}\}/g, ' ').toLowerCase();
