...
import { renderDe, lockup } from './marca-tuu.js';
...
export function htmlDeTarjeta(pieza) {
  ...
  // Material oficial de TUU, si esta disponible. Sin el, la pieza sale como
  // antes: solo texto sobre el azul.
  const render = pieza.producto_handle ? renderDe(pieza.producto_handle) : '';
  const marcaTuu = lockup();
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  /* El equipo va sobre blanco, nunca sobre el azul: las maquinas TUU son
     azules y contra el fondo de marca perdian el contorno entero. */
  .equipo{position:absolute;right:56px;bottom:188px;width:392px;height:392px;
    background:${MARCA.blanco};border-radius:26px;display:flex;
    align-items:center;justify-content:center;box-shadow:0 18px 40px rgba(0,0,0,.2)}
  .equipo img{max-width:86%;max-height:90%;object-fit:contain}
  /* El lockup es arte blanco hecho para ir sobre el azul de TUU: va directo,
     no dentro de una pastilla que repita lo mismo. */
  /* width:auto y align-self son obligatorios: .marco es un flex en columna y
     estira a sus hijos a todo el ancho, lo que deformaba el lockup. */
  .lockup{height:62px;width:auto;align-self:flex-start;display:block;margin-bottom:40px}
  ...
  <div class="marco">
    ${marcaTuu
      ? `<img class="lockup" src="${marcaTuu}" alt="TUU Chile, distribuidor autorizado">`
    ...
    ${producto
      ...
      : `<div class="cierre">
          ...
          <div class="canal">Instagram @${esc(config.negocio.instagram)} · WhatsApp</div>
        </div>`}
  </div>
  ${render ? `<div class="equipo"><img src="${render}" alt=""></div>` : ''}
  <div class="pie">
    <div class="marca">
